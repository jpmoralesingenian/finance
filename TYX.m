...
debug_on_error(0);
[dates,open,high,low,close,volume,close2]= textread("INDEX_TYX.csv","%s %f %f %f %f %f %f",'delimiter',',','headerlines',1);
dates2 = zeros(0,0);
sums = zeros(0,0);
counts = zeros(0,0);
...
% Yearly rolling average
	if(a<number_of_periods-12)
		averages(a) = mean(averages(a:a+12));
	else 
		averages(a) = mean(averages(a:number_of_periods));
	endif
endfor
% 
